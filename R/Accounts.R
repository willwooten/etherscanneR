...
get_internal_txs_by_blockrange <- function(start_block = 0, end_block = 99999999, sort = "asc", page = 1){
  url <- paste0("https://api.etherscan.io/api?module=account&action=txlistinternal&startblock=", start_block, "&endblock=", end_block, "&page=", page, "&sort=asc&apikey=", api_key)
  jsonlite::fromJSON(url)$result
}
...
get_erc20_transfers_by_address <- function(address, contract, api_key, start_block = 0, end_block = 99999999, sort = "asc"){
  url <- paste0("https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=", contract ,"&address=", address, "&startblock=", start_block ,"&endblock=", end_block, "&sort=", sort, "&apikey=", api_key)
  jsonlite::fromJSON(url)$result
}

# Get a list of "ERC721 - Token Transfer Events" by Address

get_erc721_transfers_by_address <- function(address, contract, api_key, start_block = 0, end_block = 99999999, sort = "asc"){
  url <- paste0("https://api.etherscan.io/api?module=account&action=tokennfttx&contractaddress=", contract ,"&address=", address, "&startblock=", start_block ,"&endblock=", end_block, "&sort=", sort, "&apikey=", api_key)
  jsonlite::fromJSON(url)$result
}
...
get_blocks_mined_by_address <- function(address, api_key, block_type = "blocks",start_block = 0, end_block = 99999999, sort = "asc", page = 1){
  url <- paste0("https://api.etherscan.io/api?module=account&action=getminedblocks&address=", address, "&blocktype=", block_type, "&startblock=", start_block ,"&endblock=", end_block, "&page=", page, "&sort=", sort, "&apikey=", api_key)
  jsonlite::fromJSON(url)$result
}
